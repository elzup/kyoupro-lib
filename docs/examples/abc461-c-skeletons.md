# 例題ドリル（多言語）: ABC461 C — 解法→コーディング雛形

**「解法は思いついた。あとはコードにできるか？」**を訓練するための穴埋め。
方針の解説は [`abc461-c-variety.md`](./abc461-c-variety.md)、変数の動きは [`abc461-c-variable-trace.html`](./abc461-c-variable-trace.html) を参照。

ルール: 入出力（I/O）は雛形で用意済み。各工程の **目標状態** を見て、その状態に到達するコードを `TODO` に書く。
