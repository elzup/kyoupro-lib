# 例題講義: ABC461 C - Variety 〜「イメージをコードにする」実装トレーニング

`docs/solving-flow.md` の **実装フェーズ（＝言語理解／実装力 B）** を 1 問で鍛える講義。
アイデア（方針）は早めに固め、それを具体的なイメージ（変数の状態と遷移）にしてから、**そのイメージをどうコードに落とすか**を段階的に組み立てる。
...
難易度的には Level 3〜4 の橋。設計（貪欲の発想 = A）より、**作ったイメージを素早くコードにする力**を測る回。
